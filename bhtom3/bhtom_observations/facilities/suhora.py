...
#    observation_params = forms.CharField(required=False, widget=forms.Textarea(attrs={'type': 'json'}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        dynamic_choices = get_proposal_choices_for_user(self.initial.get('request_user_id'), 'SUHORA', include_account_label=True)
        if dynamic_choices:
...
[ENDSUHORAOB]
        ...
        selected_proposal = get_proposal_by_pk(observation_payload['params'].get('proposal_id'), facility_code='SUHORA')
        if not selected_proposal:
            raise ValueError('Selected SUHORA proposal is not available in the proposal database.')
        proposal_id = selected_proposal.external_id
        email = (selected_proposal.details or {}).get('notification_email', '').strip()
        if not email:
            raise ValueError('SUHORA proposal is missing notification email.')
