...
class ATLASQueryForm(BaseQueryForm):
    ...
    ra = ra_field()
    dec = dec_field()
    mjd_min = forms.FloatField(required=False, label='MJD min (optional)')
    mjd_max = forms.FloatField(required=False, label='MJD max (optional)')
    include_photometry = forms.BooleanField(required=False, initial=True, label='Include photometry')
...
class SCATQueryForm(BaseQueryForm):
    target_name = target_name_field()
    ra = ra_field()
    dec = dec_field()
    radius_arcsec = forms.FloatField(required=False, initial=10.0, min_value=0.01, label='Search radius (arcsec)')
    include_spectroscopy = forms.BooleanField(required=False, initial=True, label='Include spectroscopy')
...
class HipparcosQueryForm(BaseQueryForm):
    ...
    ra = ra_field()
    dec = dec_field()
    radius_arcsec = forms.FloatField(required=False, initial=5.0, min_value=0.1, max_value=300.0,
                                     label='Search radius (arcsec)')
    include_photometry = forms.BooleanField(required=False, initial=True, label='Include photometry')
