...
def _build_wise_query(ra,dec,rad):
    ...
    dec = str(dec)
    rad = str(rad)
    return ("https://irsa.ipac.caltech.edu/cgi-bin/Gator/nph-query?catalog=neowiser_p1bs_psd&spatial=cone&radius"
                 "="+rad+"&radunits=arcsec&objstr=") + ra + "+" + dec + ("&outfmt=1&selcols=ra,dec,mjd,w1mpro,"
                                                                           "w1sigmpro,w2mpro,w2sigmpro")
...
class NeoWISEDataService(DataService):
    ...
    def query_targets(self, query_parameters, **kwargs):
        ...
        return [{
            ...
            'ra': ra,
            'dec': dec,
            'aliases': [{'name': alias_name, 'source_name': 'AllWISE' if data.get('alias') else self.name}],
